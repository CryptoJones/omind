...
| D1 | `omind guard selftest` | **Every** harness row `[ok]`, and exit 0. Rows: claude/exit2, hermes/claude_json, opencode/json_signal, codex/codex_hook, gemini/gemini, deepseek/json_signal, poolside/poolside, openclaw/openclaw, agy/agy. Beware measuring `$?` through a pipe (`selftest \| head` reports *head's* status) — redirect instead |
