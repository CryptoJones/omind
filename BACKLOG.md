...
- [ ] **Guard hook: substring match on escalation keywords causes false positives** ([#98](https://github.com/CryptoJones/omind/issues/98)) — _bug_ — the Bash guard hook (`hooks/omi-guard.sh`) substring-matches escalation keywords and blocks benign commands. Same root cause as #108 — to be fixed as one PR (command-position anchoring in `policy.py`).
- [ ] **Guard rule false-positives on escalation token anywhere in command** ([Codeberg #94](https://codeberg.org/CryptoJones/omind/issues/94), [GitHub #108](https://github.com/CryptoJones/omind/issues/108)) — _bug_ — the `TIER_SUDO` policy rules match the privilege keyword as a word-bounded token anywhere in the command (grep args, commit messages, `pass show` paths, the sanctioned `fleet-sudo --entry` value), not just in command position. Fix: anchor both rules to command position (`_CMD_POSITION`). Lands together with #98.
- [ ] **`omind setup` wedges the agent on machines without `jq`** ([Codeberg #93](https://codeberg.org/CryptoJones/omind/issues/93), [GitHub #107](https://github.com/CryptoJones/omind/issues/107)) — _bug_ — on a host without `jq`, `omind setup` provisions the `"*"` fail-closed OMI guard whose only dep is `jq`, blocking every subsequent tool call including the one to install `jq`. Fix: route the hook event through the pure-Python `omind guard adapter` when `jq` is missing (keeps enforcement); add a warn-level `jq` check to `doctor`.
