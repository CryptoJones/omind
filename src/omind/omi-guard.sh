...
case "$tool" in
  mcp__omi__*) mkdir -p "$STATE" 2>/dev/null; : > "$SENT" 2>/dev/null; exit 0 ;;
esac
if [ "$tool" = "Read" ]; then
  fp="$(printf '%s' "$input" | jq -r '.tool_input.file_path // empty' 2>/dev/null)"
  case "$fp" in
    *"$OMI_DIR"*) mkdir -p "$STATE" 2>/dev/null; : > "$SENT" 2>/dev/null; exit 0 ;;
  esac
fi
...
# All other tools: the per-turn gate, in bash (fast — no subprocess).
[ -e "$SENT" ] && exit 0
